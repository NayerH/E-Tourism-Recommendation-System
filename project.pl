...
offerAccommodation(offer(taba, [diving], 1000, 2020-02-12, 2020-03-12, period(2020-06-01, 2020-08-31), 10, 1), hotel).
...
recommendOffer(Customers, PreferenceList, O, CusChosen):-
	offerMean(O,_),
	recommendOfferHelper(Customers, PreferenceList, O, [], ListWithRating),
	sortList(ListWithRating, SortedList),
	reverse(SortedList, ReverseList),
	choose(O, [], ReverseList, CusChosen).

recommendOfferHelper([], [], _, Res, Res).
recommendOfferHelper([HeadC|TailC], [HeadP|TailP], O, Acc, ChosenCustomers):-
	recommendOfferForCustomer(HeadP, ChosenPreference, O),
	preferenceSatisfaction(O, HeadC, ChosenPreference, Rate),
	append([(HeadC,Rate)], Acc, NewAcc),
	recommendOfferHelper(TailC, TailP, O, NewAcc, ChosenCustomers).

sortList([], []).
sortList([A|B], Sorted) :- sortList(B, SortedTail), insertSort(A, SortedTail, Sorted).
insertSort((CusH,RateH), [], [(CusH,RateH)]).
insertSort((CusH,RateH), [(CusT,RateT)|Tail], [(CusT,RateT)|NewTail]):-
	RateH > RateT,
	insertSort((CusH,RateH), Tail, NewTail).
insertSort((CusH,RateH), [(CusT,RateT)|Tail], [(CusH,RateH), (CusT,RateT)|Tail]).

reverse([], []).
reverse([H|T], RevList):-
    reverse(T, RevTail), append(RevTail, [H], RevList).

choose(offer(_, _, _, _, _, _, _, 0), Acc, _, Acc).
choose(_, Acc, [], Acc).
choose(offer(_, _, _, _, _, _, _, N), Acc, [(H,_)|T], ChosenCustomers):-
	N \= 0,
	append([H],Acc,NewAcc),
	G is N-1,
	choose(offer(_, _, _, _, _, _, _, G), NewAcc, T, ChosenCustomers).
...
mostPreferredActivity(C,A):-
	getAllActivities(L),
	L = [H|T],
	mostPreferredActivityH(C,A,[H|T],H).

mostPreferredActivityH(_,Acc,[],Acc).
mostPreferredActivityH(C,A,[H|T],Acc):-
	customerPreferredActivity(C,H,RCurrH),
	customerPreferredActivity(C,Acc,RCurrAcc),
	RCurrH >= RCurrAcc,
	mostPreferredActivityH(C,A,T,H).
mostPreferredActivityH(C,A,[H|T],Acc):-
	customerPreferredActivity(C,H,RCurrH),
	customerPreferredActivity(C,Acc,RCurrAcc),
	RCurrH < RCurrAcc,
	mostPreferredActivityH(C,A,T,Acc).